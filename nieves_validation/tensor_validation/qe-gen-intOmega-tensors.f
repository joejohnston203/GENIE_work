...
c     This program integrates over omega=cos(theta_lepton) in order to get 
c     dSigma/dElepton. It is a modification of Nieves' qe.f code.
      
      IMPLICIT REAL*8 (A-H,O-T,V-Z)
      dimension cosTheta(2000),sigArray(2000)
...
c      read(5,*) cosMin,cosMax
     ...
     f        '[10^{-38} cm^2/GeV'
      write(60,12)'Tlepton (GeV), dsigma/dTlepton, MA(GeV), ',
     f     'RPA (0/1), Eneutrino (GeV)'

      do it=1,100
c     muon kinetic energy (GeV)
      tmugev=.01+it*(eingev-0.115d0)/100.d0
...
c        Integrate over omega = cos(theta_lepton) using DSG20R and DRG20R
         ...
         write(60,11) tmugev,sig*1d-3,xmagev,ipol,eingev
      enddo
...
c     script will label and plot the results
          write(61,15) -q2,q0,dq,f1v,xmuf2v,gaq,gpq,axx,azz,a0z,a00,axy,
     f         fact,facl,f00,q2
 15       format(e15.7,e15.7,e15.7,e15.7,e15.7,e15.7,e15.7,e15.7,e15.7,
     f         e15.7,e15.7,e15.7,e15.7,e15.7,e15.7,e15.7)
c     End print
