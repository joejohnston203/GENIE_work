...
c*******************************************************************
        ...
        ncxro=0 ! normaliza densidad
         ...
         lc = 1 ! 0 desconvoluciona cualquier otro numero no
...
100          FORMAT(A)
     ...
     f               2.d0*eout*(eout+pout)*(1.d0-coseno**2)/dma**2) -
     f               0.5d0*wd(5)*(eout+pout)/dma
c	I replaced sin12**2 with (1-coseno**2) to match the paper -JJ
...
* DXPP ES IGUAL A 2/3 DEL RADIO CUADRATICO MEDIO DEL PROTON EN FM2
      ...
      double precision FUNCTION DUR (Q0,DQ,PF1,PF2,DMNU,t0,r00)
      IMPLICIT DOUBLE PRECISION (A-H,O-T,V-Z)
      IMPLICIT COMPLEX*16 (U)
       hbarc = 197.3269602d0

      IF (Q0.LT.0) THEN
       WRITE(6,*)'EN LA LLAMADA A DUR,Q0 ES NEGATIVA'
       DUR=0.
       t0=0.d0
       r00 =0.d0 
       return
...
**********************************************************************
 ...
 37    IF(abs(B/CAPC)-0.1.GT.0) THEN
	GO TO 39
	ELSE
	GO TO 38
	END IF
