...
The TH1D* pointer is returned so that it can be added to a legend
*/
...
void save_plot(TString graph_title, double ymax, TString rootFile, TString spline, TString target, double Enu,
	       TString RPA, TString Coul, double ctlminmax[], int ctlsize){
  TString EnuStr;
  EnuStr += Enu;
  EnuStr.ReplaceAll(" ","");
  TString plotTitle = "dSigma/dTl, " + target + 
    ", Enu = " + EnuStr + ", gevgen vs Nieves' fortran code";
  
  // Plot the result for the given angles
  ...
  for(int i=0;i+1<ctlsize;i+=2){
    ctlmin = ctlminmax[i];
    ctlmax = ctlminmax[i+1];
    TString ctlminStr,ctlmaxStr;
    ctlminStr += ctlmin;
    ctlminStr.ReplaceAll(" ","");
    ctlmaxStr += ctlmax;
    ctlmaxStr.ReplaceAll(" ","");
    TString nievesName = target + "_" + EnuStr + "_R" + RPA + "_C" + Coul ;
    TString nievesData = "dsigmadTl_" + target + "/" 
      + nievesName + "/" + nievesName + "_" + ctlminStr + "_" + ctlmaxStr + ".txt";
    TH1D* hst = validation_plot(ymax,rootFile,spline,target,Enu,
				  ctlmin,ctlmax,nievesData);
    leg1->AddEntry(hst,"ctl=" + ctlminStr + " to " + ctlmaxStr,"L");
  }
  leg1->Draw();
		    ...
		    int numBins = 100)
{
  ...
  TH1D* hst_Y = xsec_vs_Y(ymax,rootFile,spline,target,Enu,"El-0.106",0.0,Enu,
