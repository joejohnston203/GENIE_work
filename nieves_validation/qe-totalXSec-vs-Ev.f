...
c        Integrate over lepton energy
         CALL DSG20R(0.d0,eingev,5,tmugevArr,NPL)
         do i = 1,NPL
            tmugev = tmugevArr(i)
c           Integrate over omega = cos(theta_lepton) using DSG20R and DRG20R
            ...
            dsigdel(i) = sig
         enddo
         CALL DRG20R(0.d0,eingev,5,dsigdel,sigma)
         write(70,11) eingev,sigma*1d-3,xmagev,ipol
      enddo
      
c      write(60,13)'units diff cross section', 
c     f     '[10^{-38} cm^2/GeV/num_of_active_nucleons (N or Z)]'
 13   format ('#', 1x, a,1x,a)
