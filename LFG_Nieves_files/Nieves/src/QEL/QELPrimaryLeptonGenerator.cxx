...
/*
 ...
 @ Mar 18, 2016 - JJ (SD)
   A lepton is now generated and saved when
   QELKinematicsGenerator::ProcessEventRecordRuns(). SetRunningLepton() in
   this class generates a lepton. ProcessEventRecord() here is
   updated to access and store that lepton.
*/
//____________________________________________________________________________
...
void QELPrimaryLeptonGenerator::ProcessEventRecord(GHepRecord * evrec) const
{
  ...
  if(!(kine->KVSet(kKVSelTl) && kine->KVSet(kKVSelctl) 
    ...
    LOG("LeptonicVertex",pFATAL) << "TESTING: Parent method called";
    PrimaryLeptonGenerator::ProcessEventRecord(evrec);
    return;
  }
