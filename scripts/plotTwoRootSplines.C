...
void plotSplines(TString rootFile1, TString rootFile2, TString target,TString process = "qel_cc_n"){
  TString types[4] = {"LS_GFG","LS_LFG","N_GFG","N_LFG"};
  TString files[4];
  //for(int i=0;i<4;i++) files[i] = "rootfile_" + types[i] + ".root";
  files[0] = rootFile1;
  files[1] = rootFile2;
  Color_t colors[4] = {kBlue,kRed,kCyan,kMagenta};
  ...
  //graphs[i]->Draw("AL");
  //c1->SaveAs(files[i] + "_" + target + ".png");
  mg->Add(graphs[i], "AL");
