...
double LwlynSmithQELCCPXSec::FullDifferentialXSec(const Interaction *  interaction)const{
  ...
  std::cout << "lh qTildep4 = " << utils::print::P4AsString(&qTildeP4) << "\n"
	    << "lh neutrinop4 = " << utils::print::P4AsString(neutrinoMom) << "\n"
	    << "lh inNucleon = " << utils::print::P4AsString(inNucleonMom) << "\n"
	    << "lh lepton = " << utils::print::P4AsString(&leptonMom) << "\n"
	    << "lh outNucleon = " << utils::print::P4AsString(&outNucleonMom) << "\n";
...
//  std::cout << "tau = " << tau << ", h1 = " << h1 << ", h2 = " << h2 <<", h3 = " << h3 <<", h4 = " << h4 <<  std::endl;
  ...
  std::cout << "xsec = " << Gfactor * LH << std::endl;
  delete neutrinoMom;
  
  double xsec = Gfactor * LH;

  if( interaction->TestBit(kIAssumeFreeNucleon) ){
    std::cout << "assume free nucleon" << std::endl;
    return xsec;
  }
  ...
  std::cout << "returning nuclear xsec" << std::endl;
  return xsec;
}
